...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = loss(D_X(images_X), ones_target(Nx))
        D_Y_loss = loss(D_Y(images_Y), ones_target(Ny))
        ...
        D_X_loss = loss(D_X(fake_X), zeros_target(Nx))

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY.forward(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = loss(D_Y(fake_Y), zeros_target(Ny))
        ...
        g_loss = loss(D_X(fake_X), ones_target(Nx) )
        ...
        g_loss = loss(D_Y(fake_Y), ones_target(Ny) )
